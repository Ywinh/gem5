...
#include "systemc/tlm_bridge/c920_tlm_simple_mem.hh"
...
C920SimpleMemoryTarget::C920SimpleMemoryTarget(
    ...
    gem5::Tick latency, gem5::Tick latencyVar,
    double bandwidth) :
    ...
    transaction_type &trans, const phase_type &phase)
{
    ...
    if (phase == tlm::END_RESP) {
        if (!responseInProgress) {
            SC_REPORT_FATAL(
                "C920TlmSimpleMem", "Illegal END_RESP received by target");
        }
    ...
    SC_REPORT_FATAL(
        "C920TlmSimpleMem", "Illegal phase received by target PEQ");
}
...
C920SimpleMemoryTarget::send_end_req(transaction_type &trans)
{
    ...
    if (status != tlm::TLM_ACCEPTED) {
        SC_REPORT_FATAL(
            "C920TlmSimpleMem", "Unexpected status after sending END_REQ");
    }
}
...
C920SimpleMemoryTarget::send_response(transaction_type &trans)
{
    ...
    if (status == tlm::TLM_COMPLETED ||
        ...
        SC_REPORT_FATAL(
            "C920TlmSimpleMem", "Unexpected status after sending BEGIN_RESP");
    }
}
...
C920TlmSimpleMem::C920TlmSimpleMem(
    const Params &params, const sc_core::sc_module_name &name) :
    sc_core::sc_module(name),
    target(
        "target", params.range, params.latency, params.latency_var,
        params.bandwidth),
    wrapper(target.socket, std::string(this->name()) + ".tlm",
            gem5::InvalidPortID)
{
}

gem5::Port &
C920TlmSimpleMem::gem5_getPort(const std::string &if_name, int idx)
{
    if (if_name == "tlm") {
        return wrapper;
    }
...
sc_gem5::C920TlmSimpleMem *
gem5::C920TlmSimpleMemParams::create() const
{
    return new sc_gem5::C920TlmSimpleMem(
